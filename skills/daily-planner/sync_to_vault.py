...
def _body_content_to_markdown(body: list) -> str:
    """Convert a Google Docs API body content list to markdown."""
    paras = []
    for elem in body:
...
def _parse_gdoc_to_markdown(raw_json: str) -> str:
    """Convert raw Google Docs API JSON (default body) to markdown."""
    try:
        doc = json.loads(raw_json)
    except json.JSONDecodeError:
        return ''
    body = doc.get('body', {}).get('content', [])
    return _body_content_to_markdown(body)


def _strip_gemini_notes_content(content: str) -> str:
    """Strip header block and trailing boilerplate from a Gemini Notes tab markdown string."""
    ...
    # Strip header block: everything before the first ### heading (Summary, etc.)
    first_h3 = re.search(r'^###\s+\S', content, re.MULTILINE)
    if first_h3:
...
def update_meeting_with_gemini_notes(meeting_file: Path) -> bool:
    """Fetch Gemini Notes tab and inject/update ## Notes by Gemini section."""
...
def fetch_and_parse_transcript_tab(doc_id: str) -> Optional[str]:
    ...
    result = _run(
        ['gog', 'docs', 'cat', '--tab', 'Transcript', '--raw', '--json', '--results-only', doc_id],
        timeout=30,
    )
    if not result or result.returncode != 0 or not result.stdout.strip():
        return None
